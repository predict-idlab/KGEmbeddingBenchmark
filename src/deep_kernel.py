...
def load_data_old(ds_name):
    f = open(data_dir + "/%s.graph"%ds_name, "rb")
    print(f)
    data = pickle.load(f)
...
def load_data(ds_name):
    ...
    for i in range(num_graphs):
        ...
        for q in range(num_nodes):
            neighbors=[j for j in range(10) if j!=q]
            graph[q]={}
            graph[q]['neighbors']=neighbors
            graph[q]['label']=(i,)
        graph_data[i]=graph
    return graph_data

def load_data_rdf(ds_name):
...
def build_wl_corpus(ds_name, max_h,graph_data):
    labels = {}
    label_lookup = {}
    label_counter = 0
    vocabulary = set()
...
def compute_dgk2(graph_data,num_dimensions=1, # any integer > 0
    ...
    # STEP 1: Build corpus
    start = time.time()
    corpus, vocabulary, prob_map,  num_graphs = build_wl_corpus(ds_name, max_h,graph_data)
...
def compute_dgk(graphs,num_dimensions=1, # any integer > 0
         kernel_type=1, # 1 (deep, l2) or 2 (deep, M), 3 (MLE)
         feature_type=3, # 1 (graphlet), 2 (SP), 3 (WL)
         ds_name='nci1', # dataset name
         window_size=1, # any integer > 0
    ...
    if feature_type == 1:
        ...
        graph_data,_= fc.convert_to_dict(graphs,label_dict={},label_dict_reverse={})
        corpus, vocabulary, prob_map,  num_graphs = build_wl_corpus(ds_name, max_h,graph_data)
    else:
...
#output_dir = '../rdfdata/mutag/change_test/'
#subgraphs_dir = glob.glob(output_dir + "/*")  # extract all the subgraphs dirs
#subgraphs_dir = [output_dir + 'd1_%s.xml' % (str(i)) for i in range(0, len(subgraphs_dir))]
#complete_graph='../rdfdata/mutag/mutag.owl'
#compute_dgk(complete_graph,subgraphs_dir)